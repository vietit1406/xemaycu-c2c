				...
				<div class="wrap-table-shopping-cart bgwhite">
					<table class="table-shopping-cart table-hover">
						<tr class="table-head t-center">
							...
							<th class="column-4">Người bán</th>
							<th class="column-4">SĐT</th>
						...
						@if($lichsu->tinhtrang != "Đang xác nhận" && $lichsu->rate == "Chưa đánh giá")
						<tr class="table-row" data-href="danhgia?name={{$lichsu->name}}&matin={{$lichsu->matin}}" style="cursor: pointer;">
						@else
						<tr class="table-row" data-href="lichsugiaodich" style="cursor: pointer;">
						@endif
							<td class="column-1">
								
								<div style="position: relative;width: 90px">
									<img src="source/images/tin/{{$lichsu->hinh1}}" alt="IMG-PRODUCT" style="width: 100%">
								</div>
							</td>
							<td class="column-2">{{$lichsu->tenxe}}</td>
							<td class="column-3">{{$lichsu->name}}</td>
							<td class="column-3">{{$lichsu->sdt}}</td>
							<td class="column-4">{{$lichsu->ngaydh}}</td>
							<td class="column-5">{{number_format($lichsu->tongtien)}} VNĐ</td>

							@if($lichsu->rate == "Chưa đánh giá" && $lichsu->tinhtrang != "Đang tiến hành")
							<td class="column-6"><font color="red">{{$lichsu->tinhtrang}}</font></td>
							@else
							<td class="column-6">{{$lichsu->tinhtrang}}</td>
							@endif

							@if($lichsu->rate == "Chưa đánh giá")
							<td class="column-7"><font color="red">{{$lichsu->rate}}</font></td>
							@else
							<td class="column-7">{{$lichsu->rate}}</td>
							@endif
						</tr>
						@endforeach
						@endif
